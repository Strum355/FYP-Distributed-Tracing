    ...
    \chapter{Design \& Implementation}
        ...
        \section{Architecture Design}
            ...
            \subsection{Debug Adapter}
                ...
                at the span level. In both these instances, the data is attached to spans as tags. There are a few downsides to this approach and possible workarounds that will be discussed.
                ...
                of the stacktraces. Another downside to full stacktraces is that the procedure for getting an entire stacktrace and transforming them can often be considerably more expensive than 
                adding simple tags to a span. Unfortunately, sampling rules, the processes by which it is decided whether or not spans are exported, in the Jaeger client implementations are only 
                applied to log points and the final reporting of spans. This results in the potentially expensive operation of gathering stacktraces to be run even if the span will never be reported.
                Despite implementing head-based sampling, where it is know when the root span is created whether or not the trace will be reported, the API does not expose an interface for querying this
                fact. For this reason, it was important to make the operation as cheap as possible, leaving as much stacktrace transformations as possible to query time, when the debugger runtime requests
                a trace from the backend.
